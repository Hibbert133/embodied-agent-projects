"""Generate the immutable SciAgent Probe Value v1.4 manifest."""
...
DEFAULT_CONFIG = ROOT / "configs/probemem_sciagent/api_reliability_v1_4.json"
...
def build_units(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if (start, stop) != (6300, 6349) or len(units) != 50:
        raise ValueError("Probe Value v1.4 requires seeds 6300--6349")
...
def main() -> int:
    ...
    try:
        ...
        run_id = f"probemem_sciagent_probe_value_{stamp}_{commit[:12]}"
        implementation = [
            ...
            ROOT / "src/probemem_sciagent/probe_value.py",
            ...
            ROOT / "scripts/generate_probemem_sciagent_probe_value_manifest.py",
            ROOT / "scripts/run_probemem_sciagent_api_reliability.py",
        ]
        inputs = [
            config_path,
            ROOT / config["seed_registry"],
            ROOT / "docs/protocols/probemem_sciagent_probe_value_v1_4.md",
        ]
        manifest = {
