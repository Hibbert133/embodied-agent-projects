"""Generate the immutable SciAgent API Reliability v1.1 shadow manifest."""
...
DEFAULT_CONFIG = ROOT / "configs/probemem_sciagent/api_reliability_v1_1.json"
...
def build_units(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if (start, stop) != (5850, 5899) or len(units) != 50: raise ValueError("API Reliability v1.1 requires seeds 5850--5899")
    if any(row["initial_seed"] == row["mandatory_probe_seed"] for row in units): raise ValueError("random namespaces overlap")
    return units
...
def main() -> int:
    ...
    try:
        ...
        run_id = f"probemem_sciagent_api_reliability_{stamp}_{commit[:12]}"
        implementation = [
            ...
            ROOT / "scripts/generate_probemem_sciagent_api_reliability_manifest.py",
            ROOT / "scripts/run_probemem_sciagent_api_reliability.py",
            ROOT / "scripts/analyze_probemem_sciagent_api_reliability.py",
        ]
        inputs = [config_path, ROOT / config["seed_registry"], ROOT / "docs/protocols/probemem_sciagent_api_reliability_v1_1.md"]
        manifest = {
        ...
        output = ROOT / config["output_root"] / run_id; output.mkdir(parents=True, exist_ok=False)
        path = output / "immutable_manifest.json"; path.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print(f"manifest: {path}"); return 0
