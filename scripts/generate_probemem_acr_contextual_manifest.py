"""Generate an immutable manifest for contextual ACR development."""
...
CONFIG_PATH = Path("configs/probemem_acr/contextual_utility_development_v1.json")
IMPLEMENTATION_PATHS = (
    Path("scripts/check_probemem_acr_contextual_seeds.py"),
    Path("scripts/collect_probemem_acr_distributional_stream.py"),
    Path("scripts/replay_probemem_acr_contextual_methods.py"),
    Path("scripts/run_probemem_acr_utility_stability.py"),
    Path("src/probemem/contextual_policy.py"),
...
def main() -> int:
    try:
        if _git("status", "--porcelain"):
            raise RuntimeError("contextual manifest requires a clean worktree")
        subprocess.run(
            [sys.executable, str(ROOT / "scripts/check_probemem_acr_contextual_seeds.py")],
            cwd=ROOT, check=True,
        )
        config = json.loads((ROOT / CONFIG_PATH).read_text(encoding="utf-8"))
        start, stop = config["seed_partitions"]["development"]
        if [start, stop] != [2500, 2699]:
            raise ValueError("contextual development seeds must remain 2500--2699")
        ...
        run_id = f"acr_contextual_{timestamp}_{commit[:12]}"
        manifest: dict[str, Any] = {
            ...
            "contextual_model_hash": _hash(config["contextual_model"]),
            "promotion_gate_hash": _hash(config["promotion_gate"]),
            "claim_scope": config["claim_scope"],
            "predecessor_run": "acr_distributional_v2_20260802T131001Z_6178437d677a",
        }
        manifest["manifest_id"] = _hash(manifest)
        run_dir = ROOT / "outputs/probemem_acr/contextual_runs" / run_id
