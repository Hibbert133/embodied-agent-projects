"""Generate an immutable manifest for distributional ACR development."""
...
CONFIG_PATH = Path("configs/probemem_acr/distributional_memory_development_v1.json")
IMPLEMENTATION_PATHS = (
    Path("scripts/check_probemem_acr_distributional_seeds.py"),
...
def main() -> int:
    try:
        if _git("status", "--porcelain"):
            raise RuntimeError("distributional manifest requires a clean worktree")
        subprocess.run([sys.executable, str(ROOT / "scripts/check_probemem_acr_distributional_seeds.py")], cwd=ROOT, check=True)
        config = json.loads((ROOT / CONFIG_PATH).read_text(encoding="utf-8"))
        start, stop = config["seed_partitions"]["development"]
        if [start, stop] != [2000, 2099]:
            raise ValueError("distributional development seeds must remain 2000--2099")
        ...
        run_id = f"acr_distributional_{timestamp}_{commit[:12]}"
