"""Generate the immutable SciAgent Quantized Probe Value v1.5 manifest."""
...
DEFAULT_CONFIG = ROOT / "configs/probemem_sciagent/api_reliability_v1_5.json"
...
def build_units(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if (start, stop) != (6450, 6499) or len(units) != 50:
        raise ValueError("Quantized Probe Value v1.5 requires seeds 6450--6499")
...
def main() -> int:
    ...
    try:
        ...
        run_id = f"probemem_sciagent_quantized_value_{stamp}_{commit[:12]}"
        implementation = [
            ROOT / "src/probemem_sciagent/api_reliability.py",
            ROOT / "src/probemem_sciagent/api_envelope.py",
            ROOT / "src/probemem_sciagent/capability_contract.py",
            ROOT / "src/probemem_sciagent/quantized_probe_value.py",
            ...
            ROOT / "scripts/generate_probemem_sciagent_quantized_probe_value_manifest.py",
            ROOT / "scripts/run_probemem_sciagent_api_reliability.py",
        ]
        inputs = [
            config_path, ROOT / config["seed_registry"],
            ROOT / "docs/protocols/probemem_sciagent_quantized_probe_value_v1_5.md",
        ]
        manifest = {
        ...
        (output / "immutable_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
        print(f"manifest: {output / 'immutable_manifest.json'}")
        return 0
