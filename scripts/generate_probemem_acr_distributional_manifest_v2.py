"""Generate an immutable manifest for distributional ACR development v2."""
...
CONFIG_PATH = Path("configs/probemem_acr/distributional_memory_development_v2.json")
IMPLEMENTATION_PATHS = (
    Path("scripts/check_probemem_acr_distributional_seeds_v2.py"),
    Path("scripts/collect_probemem_acr_distributional_stream.py"),
    Path("scripts/replay_probemem_acr_distributional_methods.py"),
    Path("scripts/run_probemem_acr_utility_stability.py"),
    Path("src/probemem/distributional_policy.py"),
...
def main() -> int:
    try:
        if _git("status", "--porcelain"):
            raise RuntimeError("distributional v2 manifest requires a clean worktree")
        subprocess.run(
            [sys.executable, str(ROOT / "scripts/check_probemem_acr_distributional_seeds_v2.py")],
            cwd=ROOT, check=True,
        )
        config = json.loads((ROOT / CONFIG_PATH).read_text(encoding="utf-8"))
        start, stop = config["seed_partitions"]["development"]
        if [start, stop] != [2200, 2349]:
            raise ValueError("distributional v2 development seeds must remain 2200--2349")
        ...
        run_id = f"acr_distributional_v2_{timestamp}_{commit[:12]}"
        manifest: dict[str, Any] = {
            ...
            "policy_hash": _hash(config["distributional_policy"]),
            "promotion_gate_hash": _hash(config["promotion_gate"]),
            "claim_scope": config["claim_scope"],
            "predecessor_incomplete_run": "acr_distributional_20260802T125208Z_ce7d9daa5547",
        }
        manifest["manifest_id"] = _hash(manifest)
        run_dir = ROOT / "outputs/probemem_acr/distributional_runs" / run_id
